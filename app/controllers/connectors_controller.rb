class ConnectorsController < ApplicationController
  ...
  # before_action :set_wiring_harness, only: %i[ create edit update destroy ]
  ...
  def create
    ...
    respond_to do |format|
      if @connector.save
        format.html { redirect_to connector_url(@connector), notice: "Connector was successfully created." }
  ...
  def update
    respond_to do |format|
      if @connector.update(connector_params)
        format.html { redirect_to connector_url(@connector), notice: "Connector was successfully updated." }
  ...
  private
    ...
    def set_wiring_harness
      @wiring_harness = WiringHarness.find(params[:connector][:wiring_harness_id])
    end

    # Only allow a list of trusted parameters through.
    def connector_params
      params.require(:connector).permit(:name, :part_number, :company, :positions, :terminal_id, :prepop_id)
    end
end
